...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const document = await getPublishedDocumentBySlug("BLOG", decodeURIComponent(slug));
  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-4.2rem)] max-w-3xl px-6 py-16">
      <p className="text-xs font-medium tracking-[0.28em] text-[var(--app-muted)] uppercase">Blog Post</p>
